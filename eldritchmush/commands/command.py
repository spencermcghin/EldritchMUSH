    ...
    def func(self):
        ...
        if bow not in (0,1):
            ...
            if hasMelee:
                ...
                if bow:
                    self.caller.msg("|020You have equipped your bow.|n")
        ...
        if melee not in (0,1):
            ...
            if hasBow:
                ...
                if melee and hasWeapon:
                    self.caller.msg("|020You are now ready to fight.|n")
...
class SetShield(Command):
    ...
    def func(self):
        ...
        # Quippy message when setting a shield as 0 or 1.
        if shield:
            self.caller.msg("|020You now have a shield.|n")
...
class SetTwoHanded(Command):
    ...
    def func(self):
        ...
        if twohanded:
            self.caller.msg("|020Your have equipped a two-handed melee weapon.|n")
        else:
    ...
    def func(self):
        ...
        if (- 3 <= target_body <= 0) and medicine:
            ...
            if target == self.caller:
                self.caller.location.msg_contents(f"|015{self.caller} pulls bandages and ointments from their bag, and starts to mend their wounds.\n|y{self.caller} heals |020{medicine}|n |ybody points per round as long as their work remains uninterrupted for the round.|n")
            elif target != self.caller:
                self.caller.location.msg_contents(f"|015{self.caller.key} comes to {target.key}'s rescue, healing {target.key}.|n\n|y{self.caller.key} heals {target.key} for|n |020{medicine}|n |ybody points per round as long as their work remains uninterrupted for the round.|n")
...
class CmdStabilize(Command):
    ...
    def func(self):
        ...
        if (- 3 <= target_body <= 0) and stabilize:
            # Return message to area and caller
            if target == self.caller:
                self.caller.location.msg_contents(f"|015{self.caller} pulls bandages and ointments from their bag, and starts to mend their wounds.\n|y{self.caller} heals|n |020{stabilize}|n |ybody points per round as long as their work remains uninterrupted.|n")
            elif target != self.caller:
                self.caller.location.msg_contents(f"|015{self.caller.key} comes to {target.key}'s rescue, healing {target.key} for|n |020{stabilize}|n |015body points.|n")
        # Apply stabilize to other target
    ...
    def func(self):
        ...
        def func(self):
            ...
            if battlefieldmedicine:
                self.caller.msg("|020You have activated the battlefield medicine ability.|n")
            else:
...
class SetWeakness(Command):
    ...
    def func(self):
        ...
        if weakness:
            ...
            self.caller.msg("|020Your weakened state has subsided.|n\n|yYou may now run and use your active martial skills.|n")
