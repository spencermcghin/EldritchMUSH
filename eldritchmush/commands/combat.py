    ...
    def func(self):
        ...
        if not hasBow:
            ...
            if attack_result >= target.db.av:
                ...
                # No target armor so subtract from their body total and hit a limb. Add logic from handler above. Leave in body handler in combat handler.
                self.caller.location.msg_contents(f"|025{self.caller.key} lets loose an arrow |n(|400{attack_result}|n)|025 at |n|020{target.key}|n|025 {target.db.av}, but it misses.|n")
