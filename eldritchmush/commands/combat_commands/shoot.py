    ...
    def func(self):
        ...
        if self.caller.db.combat_turn:
            ...
            if combat_stats.get("bow", False):
                ...
                if h.canFight(self.caller):
                    if h.isAlive(target):
                        if attack_result >= target.db.av:
                            self.caller.location.msg_contents(f"|025{self.caller.key} lets loose an arrow |n(|020{attack_result}|n)|025 straight for {target.key}'s {shot_location} and hits|n (|300{target.db.av}|n), |025dealing|n |4301|n |025damage!|n")
                ...
                self.caller.msg("|430Before you can shoot, you must first equip your bow using the command equip <bow type>.|n")
