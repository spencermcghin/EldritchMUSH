...
#         self.db.turn_actions[dbref] = [("flee", 0, 0, character, None),
#                                        ("flee", 0, 0, character, None)]
...
#
#     def add_action(self, action, attack_result, damage, character, target):
#         """
#         Called by combat commands to register an action with the handler.
#
#          action - string identifying the action, like "hit" or "parry"
#          attack_result - attack roll of character
#          damage - based on one or two handed weapon
#          character - character adding the action
...
#         a tuple (action, attack_result, damage, character, target).
#         """
#         # Get character id to count number of actions for character
#         chracter_id = character.id
#
#         # Character action count
#         count = self.db.action_count[chracter_id]
#
#         if 0 <= count <= 1: # only allow 2 actions
#             self.db.turn_actions[chracter_id][count] = (action, attack_result, damage, character, target)
#             actions = self.db.turn_actions
#             self.msg_all(actions)
#         else:
#             # report if we already used too many actions
#             return False
#         self.db.action_count[chracter_id] += 1
#         return True
#
...
#                 self.db.turn_actions[character.id] = [("flee", 0, 0, character, None),
#                                                       ("flee", 0, 0, character, None)]
